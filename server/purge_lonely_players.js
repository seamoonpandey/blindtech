...
async function eliminateLonelyPlayers() {
  ...
  try {
    ...
    // 3. Optional: Sync Round 7 state if it's already running
    // This is useful if the game is already in the Final Round
    const r7Check = await client.query("SELECT id FROM hearts_game_state WHERE status != 'finished'");
    if (r7Check.rows.length > 0) {
      console.log('Round 7 is active. Removing purged players from hearts_players monitor...');
