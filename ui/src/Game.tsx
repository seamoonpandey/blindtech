...
interface Round5Turn {
  player_id: string;
  round_number: number;
  card_selected: string;
  pact_used: string | null;
  is_revealed: boolean;
  team: 'A' | 'B';
}
...
interface Round6Vote {
  voter_id: string;
  target_id: string;
  used_safety: boolean;
}

interface Round6HistoryEntry {
  target_name: string;
...
interface Round6State {
  ...
  players: Round6Player[];
  votes: Round6Vote[];
  history: Round6HistoryEntry[];
}

interface Round7Player {
  user_id: string;
  name: string;
  hearts: number;
  is_alive: boolean;
              ...
              <div key={m.id} className="match-card" style={{ border: m.status === 'active' ? '2px solid #00ff00' : '2px solid #ccc', opacity: m.status === 'finished' ? 0.6 : 1, background: m.status === 'finished' ? '#f5f5f5' : 'white' }}>
                ...
                {m.status === 'finished' && (
                  <div style={{ marginTop: '1rem', textAlign: 'center', fontSize: '0.85rem', color: '#333', fontWeight: 'bold', padding: '5px', background: 'rgba(0,0,0,0.05)' }}>
                    {(() => {
                        const p1 = Array.isArray(m.team1_scores) ? m.team1_scores.filter((s: boolean) => s === true).length : 0;
...
function Round5PlayerView({ game, userId, onSelectCard }: { 
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const revealed = currentRoundTurns.length > 0 && currentRoundTurns.every((t: Round5Turn) => t.is_revealed);
  ...
  const currentRoundTurns = game.turns?.filter((t: Round5Turn) => t.round_number === game.current_round) || [];
  const myTurn = currentRoundTurns.find((t: Round5Turn) => t.player_id === userId);
  const bothSubmitted = currentRoundTurns.length === 2;
  const revealed = bothSubmitted && currentRoundTurns.every((t: Round5Turn) => t.is_revealed);
        ...
        <div style={{ textAlign: 'center', background: '#222', color: 'white', padding: '1.5rem', borderRadius: '8px', border: '4px solid #00ff00', animation: 'pulse 2s infinite' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', marginTop: '1rem' }}>
            <div>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{currentRoundTurns.find((t: Round5Turn) => t.team === 'A')?.card_selected}</div>
              {currentRoundTurns.find((t: Round5Turn) => t.team === 'A')?.pact_used && (
                <div style={{ fontSize: '0.6rem', color: '#ff4444' }}>PACT: {currentRoundTurns.find((t: Round5Turn) => t.team === 'A')?.pact_used}</div>
              )}
            </div>
            <div style={{ fontSize: '2rem', color: '#444' }}>VS</div>
            <div>
              <div style={{ fontSize: '0.7rem', color: '#aaa' }}>TEAM B</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{currentRoundTurns.find((t: Round5Turn) => t.team === 'B')?.card_selected}</div>
              {currentRoundTurns.find((t: Round5Turn) => t.team === 'B')?.pact_used && (
                <div style={{ fontSize: '0.6rem', color: '#ff4444' }}>PACT: {currentRoundTurns.find((t: Round5Turn) => t.team === 'B')?.pact_used}</div>
              )}
            </div>
          </div>
      ...
      <div style={{ marginTop: '2.5rem', borderTop: '4px solid black', paddingTop: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {Array.isArray(game.turns) && game.turns
            .filter((t: Round5Turn) => t && t.is_revealed)
            .sort((a: Round5Turn, b: Round5Turn) => (b.round_number || 0) - (a.round_number || 0))
            .map((t: Round5Turn, _: number, arr: Round5Turn[]) => {
            // Group turns by round
            if (t.team === 'B') return null; // We'll process A and find B
            const roundNum = t.round_number;
            const turnA = t;
            const turnB = arr.find((alt: Round5Turn) => alt.round_number === roundNum && alt.team === 'B');
          ...
          {(!Array.isArray(game.turns) || game.turns.filter((t: Round5Turn) => t.is_revealed).length === 0) && (
                ...
                <span className="value">
                  ...
                  {gameState.current_round >= 2 && (
                    <span style={{ fontSize: '0.6em', marginLeft: '6px', opacity: 0.8 }}>
                      ({adminTeams.filter((t: AdminTeam) => !t.user1_eliminated && !t.user2_eliminated).length} Teams)
                    </span>
                  )}
                </span>
              </div>
              ...
              <div className="card control-card">
                ...
                <div className="btn-group-vertical">
                   {gameState.current_round === 0 && (
                     <button onClick={() => onStartRound(1)} className="admin-btn primary">START ROUND 1</button>
                   )}
                   
                   {gameState.status === 'active' && gameState.current_round >= 1 && gameState.current_round <= 6 && (
                     <button 
                       onClick={() => {
                         if (gameState.current_round === 1 || gameState.current_round === 2) onFinishRound(gameState.current_round);
            ...
            <div className="admin-management-grid">
              <div className="card">
                ...
                <div className="admin-user-scroll">
                  {adminUsers.filter((u: AdminUser) => u.role === 'player').map((u: AdminUser) => (
            ...
            <div className="admin-data-content">
              ...
              {dataSubTab === 'r4' && (
                <>
                  ...
                  <div className="admin-user-list">
                    {round4Sessions.length > 0 ? round4Sessions.map((s: Round4Session) => {
  ...
  return (
    <div style={{ marginTop: '1rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
        {(games || []).map(g => {
          const currentRoundTurns = g.turns?.filter((t: Round5Turn) => t.round_number === g.current_round) || [];
          const bothSubmitted = currentRoundTurns.length === 2;
          const isRevealed = bothSubmitted && currentRoundTurns.every((t: Round5Turn) => t.is_revealed);

          const turnA = currentRoundTurns.find((t: Round5Turn) => t.is_revealed && t.team === 'A');
          const turnB = currentRoundTurns.find((t: Round5Turn) => t.is_revealed && t.team === 'B');
...
export default function Game() {
  ...
  const [round4Sessions, setRound4Sessions] = useState<Round4Session[]>([]);
  const [round5Games, setRound5Games] = useState<Round5Game[]>([]);
  const [round6State, setRound6State] = useState<Round6State | null>(null); // ROUND 6 STATE (VOTING)
  const [round7State, setRound7State] = useState<Round7State | null>(null); // ROUND 7 STATE (HEARTS)
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === 'init') {
        ...
        const matches = data.matches as Round3Match[];
        ...
        const sessions = data.sessions as Round4Session[];
        ...
        const games = data.games as Round5Game[];
  ...
  useEffect(() => {
    if (players.length > 0 && initialSubmission) {
      ...
      setTimeout(() => setSelectedPlayers(mapped), 0);
      // Only do this once
      setTimeout(() => setInitialSubmission(null), 0);
    }
      ...
      <div className="game-layout">
        ...
        <div className={`main-area ${activeTab === 'game' ? 'show' : 'hide'}`}>
          ...
          {gameState.status === 'active' && user.role === 'player' && gameState.current_round === 5 && (
            (() => {
              const myGame = round5Games.find((g: Round5Game) =>
                g.team_a_user1 === user.id || g.team_a_user2 === user.id ||
                g.team_b_user1 === user.id || g.team_b_user2 === user.id
              );              if (!myGame) {
                return (
                      ...
                      <div className="volunteer-data-list">
                        <div className="data-items">
                          {round3Matches.map(m => {
                            ...
                            const getStatusColor = (scores: boolean[] | null, isLuckyPassSlot: boolean, hasOpponent: boolean) => {
                              if (!isFinished) return 'inherit';
                              if (isLuckyPassSlot) return '#888'; 
                              if (!hasOpponent) return '#00cc00'; 
                              const s = Array.isArray(scores) ? scores : [];
                              const positives = s.filter((val: boolean) => val === true).length;
...
function Round6History({ history }: { history: Round6HistoryEntry[] }) {
        ...
        <div className="admin-card">
          ...
          {myVote && (
            <div style={{ background: '#000', color: '#fff', padding: '10px', marginBottom: '1rem', fontWeight: 'bold' }}>
              VOTE RECORDED: {state.players.find((p: Round6Player) => p.id === myVote.target_id)?.name || 'UNKNOWN'}
              {myVote.used_safety && <span style={{ color: '#00ff00', marginLeft: '10px' }}>[SAFETY ACTIVE]</span>}
            </div>
          )}
          <div className="player-list">
            {state.players.filter((p: Round6Player) => !p.is_eliminated && p.id !== userId).map((p: Round6Player) => (
              <div 
                key={p.id} 
  ...
  return (
    <div className="volunteer-view" style={{ padding: '1rem' }}>
      ...
      <div className="admin-card" style={{ marginTop: '1rem' }}>
        ...
        <div className="admin-user-list">
          {state.players.filter((p: Round6Player) => !p.is_eliminated).map((p: Round6Player) => {
             const vote = state.votes.find((v: Round6Vote) => v.voter_id === p.id);
             return (
               <div key={p.id} className="admin-user-item">
                 <div className="admin-user-info">
                   <strong>{p.name}</strong>
                   <small>{vote ? `Voted for ${state.players.find((t: Round6Player) => t.id === vote.target_id)?.name || '?'}` : 'WAITING...'}</small>
                 </div>
               </div>
             )
          })}
    ...
    <div className="card" style={{ padding: '2rem', border: '4px solid black' }}>
      ...
      <div style={{ display: 'flex', gap: '2rem', marginBottom: '2.5rem', background: '#f8f9fa', padding: '1.5rem', border: '3px solid black' }}>
        <div style={{ textAlign: 'center', flex: 1 }}>
          ...
          <div style={{ fontSize: '3rem', fontWeight: '900' }}>{'❤️'.repeat(me.hearts || 0)}</div>
        ...
        {me.teammate_id && (
          <div style={{ textAlign: 'center', flex: 1, borderLeft: '2px solid black' }}>
            ...
            <div style={{ fontWeight: 'bold' }}>{state.players.find((p:Round7Player)=>p.user_id === me.teammate_id)?.name || 'UNKNOWN'}</div>
            <div>{state.players.find((p:Round7Player)=>p.user_id === me.teammate_id)?.is_alive ? '❤️'.repeat(state.players.find((p:Round7Player)=>p.user_id === me.teammate_id)?.hearts || 0) : '💀 DEAD'}</div>
          </div>
        )}
      </div>
      ...
      {state.status === 'waiting' ? (
        ...
        <div style={{ textAlign: 'center', padding: '2rem', border: '3px dashed black' }}>
          ...
          <p style={{ fontSize: '1.5rem', margin: '1rem 0' }}>{me.current_action}: {state.players.find((p:Round7Player)=>p.user_id === me.target_id)?.name || 'NONE'}</p>
        ...
        <div>
          ...
          {others.length === 1 && state.players.filter((p:Round7Player)=>p.is_alive).length === 2 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              {me.teammate_id && state.players.find((p:Round7Player)=>p.user_id === me.teammate_id)?.is_alive && (
                <>
                  <button 
                    onClick={() => onAction('SACRIFICE', me.teammate_id || undefined)}
